...
nmouse = [797 798 828 861 882 905 906 911 912 977 994 1117 1124 1161 1162 1168 1182];
...
for i=1:length(Dir.path)
    ...
    if EpochLong
        if strcmp(Dir.name{i}, 'Mouse1161')
            UMazeEpoch{i} = or(behav{i}.SessionEpoch.Hab1,behav{i}.SessionEpoch.Hab2);
            UMazeEpoch{i} = or(UMazeEpoch{i},behav{i}.SessionEpoch.TestPre1);
        else
...
for i=1:length(Dir.path)
    ...
    for j=1:length(spikes{i}.S)
        ...
        stats_to_save{i}{j} = stats_temp{cntAll};
...
if sav
    for i=1:length(Dir.path)
        ...
        PlaceCells.stats = stats_to_save{i};
...
for i=1:length(Dir.path)
    ...
    for j=1:length(spikes{i}.S)
        ...
        try
            [map{cntAll}, ~, stats_temp{cntAll}, ~, ~, FR{cntAll}]=PlaceField_DB(spikes{i}.S{j}, ...
                behav{i}.CleanAlignedXtsd, behav{i}.CleanAlignedYtsd,'Epoch', UMazeMovingEpoch{i}, ...
                'SizeMap', sizemap, 'Smoothing', smoothing, 'PlotResults', 0, 'PlotPoisson', 0);
        catch
            stats_temp{i}{j}=[];
        end
        stats_to_save{i}{j} = stats_temp{cntAll};
